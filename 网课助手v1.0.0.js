...
function findAnswer() {
  ...
  GM_xmlhttpRequest({
    method: "GET",
    url: "http://wk.mdji.cn:8000/api?t=" + encodeURIComponent(question),
    headers: {
...
function submitAnswer($job, data) {
  ...
  data = $.grep(
    data.map(function (index) {
      ...
      if (
        !($.isPlainObject(this) && this.type < 4 && $TiMu.find(".fr").length)
      ) {
        return false;
      } else if (this.type == 2) {
        ...
        if (
          $TiMu.find(".dui").length &&
          this.code &&
          !/^A?B?C?D?E?F?G?$/.test(this.option)
        ) {
          return false;
